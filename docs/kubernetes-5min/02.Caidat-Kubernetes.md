...
- Đặt hostname cho máy `master` bằng cách sửa nội dung các file `/etc/hosts` và `/etc/hostname`
...
Chạy thử ứng dụng là ngnix để kiểm tra hoạt động của cụm cluster. Trong lệnh này sẽ tạo ra 02 container chạy nginx và tự động phân tán trên các node node1 và node2. 
...
Đối với lệnh trên ta chỉ có thể truy cập vào web server nginx sẽ được tạo trong các container bằng các địa chỉ private vì port chưa được ánh xạ ra ngoài, nếu muốn sử dụng IP của các máy k8s-master, k8s-node1, k8s-node2 thì ta cần sử dụng tùy chọn sau
...
Kiểm tra thông tin sau khi deploy
  ...
  - Có thể chuyển sang các node k8s-node1 và k8s-node2 để kiểm tra các container bằng lệnh docker ps 
