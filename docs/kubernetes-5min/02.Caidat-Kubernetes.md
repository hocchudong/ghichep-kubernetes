...
- Tới bước này, chúng ta chưa thể truy cập vào các container được, cần thực hiện thêm bước deploy các container với các tùy chọn phù hợp, cụ thể như sau
	...
	- Ngoài các tùy chọn `--port 80` thì ta lưu ý tùy chọn `--type NodePort`, đây là tùy chọn để ánh xạ port của máy cài K8S vào container vừa tạo, sử dụng các lệnh dưới để biết được port mà host ánh xạ là bao nhiêu ở bên dưới.
...
- Sử dụng các lệnh `kubectl get services` để biết được các vices được deploy với việc ánh xạ port là bao nhiêu (chính là cách xem port)
