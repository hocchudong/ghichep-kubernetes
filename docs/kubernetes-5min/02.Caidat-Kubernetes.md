...
Lựa chon 2: Nếu có một tài khoản khác tài khoản `root` thì cần thực hiện thêm các khai báo dưới để có thể sử dụng được K8S.
- Tạo user `ubuntu` để thực hiện cấu hình cho K8S.
